...
import env, { hesabfa } from "../../env";
function CustomerHolder(){
    const [stockData,setStockData]= useState();
    const [error,setError] = useState('')
    const options=["ID","شناسه محصول","توضیح محصول"];
    ...
    const getStock = () => {
        const postOptions={
            ...
            body:JSON.stringify({url:hesabfa.customerList,count:50})
        }
        //console.log(postOptions)
        setError("در حال دریافت اطلاعات حسابفا")
        fetch(env.siteApi+"/hesabfa/getApi",postOptions)
        .then(res => res.json())
        .then(
            (result) => {
            console.log(result);
